...
- **字幕自动烧录进画面（硬字幕）**：默认下载中文优先字幕并**直接烧录进视频画面**，任何播放器 / 手机打开即显示、不可关闭，不再留下散落的 `.srt/.vtt` 文件。
...
> 字幕默认以**硬字幕（烧录进画面）**方式处理：下载完的视频里字幕就是画面的一部分，任何设备直接显示、无需开 CC。若某视频在 YouTube 上本身没有字幕（官方或自动生成），则无字幕可烧，属正常。取消勾选「把字幕烧录进视频」时可退回到可选内嵌轨道模式。
