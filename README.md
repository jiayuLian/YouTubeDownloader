...
- **画质可选 / 也可不解析**：默认「标准画质(1080/720)」——有 1080p 用 1080p，没有自动降到 720p/480p；也可点「解析画质」拉取视频真实清晰度后精确选档（如 2160p / 1440p …）。
...
产物：`pyout_1/YouTubeDownloader.exe`（已内嵌 ffmpeg + yt_dlp + tcl/tk）。
...
├── build.bat            # PyInstaller 打包脚本（onefile / windowed）
